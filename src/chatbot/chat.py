...
class LumidoraChatbotConfiguration:
    def __init__(
        self, model_path: str, template: str, text_schemas: list[ResponseSchema]
    ):
        self.model_path: str = model_path
        self.template: str = template
        self.text_schemas: list[
            ResponseSchema
        ] = text_schemas  # Now handling multiple text schemas
    ...
    def from_json(cls, json_str: str):
        ...
        for schema in text_schemas_dicts:
            ...
            print(
                f"Added text schema: {text_schema.name} - {text_schema.description}"
            )  # Print each text schema

        # Create a new instance of the class using the dictionary
        return cls(
            model_path=config_dict["model_path"],
            template=config_dict["template"],
            text_schemas=text_schemas,
        )
...
class LumidoraChatbot:
    def __init__(self):
        ...
        self.callbacks: list[StreamingStdOutCallbackHandler] = [
    ...
    def run_chat(
        ...
        print(
            "Configuration loaded with model path:", config.model_path
        )  # Print model path
        ...
        self.parser: StructuredOutputParser = (
            StructuredOutputParser.from_response_schemas(config.text_schemas)
        )
        ...
        if vectorstore:
            ...
            chain_type_kwargs: dict[str, PromptTemplate] = {"prompt": prompt}
