...
class LumidoraEmbedding:
    ...
    def save_vector_db(self, vectorstore: FAISS) -> None:
    ...
    def load_vector_db(self):
        ...
        if os.path.exists(self.vectorstore_dir) and os.path.isdir(self.vectorstore_dir):
            ...
            return FAISS.load_local(
                folder_path=self.vectorstore_dir, embeddings=embeddings
            )
        else:
            # Wenn das Verzeichnis nicht existiert oder kein Verzeichnis ist, None zurückgeben
            print(
                f"Verzeichnis {self.vectorstore_dir} existiert nicht oder ist kein Verzeichnis."
            )
            return None
